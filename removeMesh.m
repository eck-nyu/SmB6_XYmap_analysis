function [newImage,allHotInds] =removeMesh(theData, theVectList, threshVal)
...
cax = [3,15];
plot_colors = {'r+','g+','b+'};
subplot(221); imagesc(log(log(1+abs(FFTimg)))), axis xy; title('FFT Before');
%caxis([0,log(1+1000*mean(abs(FFTimg(:))))]);%caxis([0,1e6])
...
subplot(222), imagesc(log(1+abs(newImage))), axis xy; title('FFT After')
...
if plot_fig==1
subplot(223), imagesc(theData), axis xy; title('Img Before');
subplot(224), imagesc(newImage), axis xy; title('Img After');
sgtitle({['threshVal = ',mat2str(threshVal/FFTimg(1,1))];['theVectList = ',mat2str(theVectList)]})
end
end
